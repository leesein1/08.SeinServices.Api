...
{
    /// <summary>
    /// MyHome API ?∞Ïù¥?∞Î? Ï°∞Ìöå??Î°úÏª¨ DBÎ°??ôÍ∏∞?îÌïòÍ≥? Î≥ÄÍ≤???Slack ?åÎ¶º???ÑÏÜ°?©Îãà??
    /// </summary>
    public class RecruitSyncService
    {
        ...
        /// <summary>
        /// Î™®ÏßëÍ≥µÍ≥† ?ôÍ∏∞?îÎ? Ï¶âÏãú 1???§Ìñâ?©Îãà??
        /// </summary>
        /// <param name="cancellationToken">Ï∑®ÏÜå ?†ÌÅ∞</param>
        /// <returns>?§Ìñâ Í≤∞Í≥º ?îÏïΩ</returns>
        ...
        {
            var title = isInsert ? "[?†Í∑ú Î™®ÏßëÍ≥µÍ≥† ?±Î°ù]" : "[Î™®ÏßëÍ≥µÍ≥† Î≥ÄÍ≤?";
            var link = string.IsNullOrWhiteSpace(item.pcUrl) ? item.url : item.pcUrl;
            return
                $"{title}\n" +
                $"- {item.pblancNm} ({item.pblancId})\n" +
                $"- ?®Ï?Î™? {item.hsmpNm}\n" +
                $"- ÏßÄ?? {item.brtcNm} {item.signguNm}\n" +
                $"- ?ÅÌÉú: {item.sttusNm}\n" +
                $"- Î™®ÏßëÍ≥µÍ≥†?? {ToDateDash(item.rcritPblancDe)}\n" +
                $"- Ï≤?ïΩÍ∏∞Í∞Ñ: {ToDateDash(item.beginDe)} ~ {ToDateDash(item.endDe)}\n" +
                $"- ?πÏ≤®?êÎ∞ú?úÏùº: {ToDateDash(item.przwnerPresnatnDe)}\n" +
                $"- ÎßÅÌÅ¨: {link}";
        }
